...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/gsd-local-patches" ]; then
    ...
    PATCHES_DIR="./desafio-fcg3/src/backend/.opencode/gsd-local-patches"
  fi
fi
...
- **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `./desafio-fcg3/src/backend/.opencode/`), variable additions (`${GSD_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
