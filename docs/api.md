...
Resumo academico do aluno. **Requer autenticacao.**
...
Arvore completa de pre-requisitos da disciplina.

### `GET /curriculum/active`
Retorna o curriculo vigente com todas as disciplinas por periodo.
  ...
  "semesters": [
    {
      "semester": 1,
      "courses": [
        {"id": "uuid", "code": "CC101", "name": "Algoritmos e Programacao", "credits": 4, "is_required": true}
      ]
    }
  ]
}
...
Retorna o periodo de matricula ativo (se houver).
...
Disciplinas disponiveis para matricula (respeita pre-requisitos). **Requer autenticacao.**
...
### `POST /enrollments`
Cria matricula (rascunho). **Requer autenticacao (student).**
...
Confirma a matricula. **Requer autenticacao (student).**

### `DELETE /enrollments/{id}/courses/{course_id}`
Remove disciplina da matricula. **Requer autenticacao (student).**

### `POST /enrollments/{id}/lock`
Tranca a matricula inteira. **Requer autenticacao (student).**
...
Notas do aluno. **Requer autenticacao.**
...
Historico escolar completo. **Requer autenticacao.**
...
### `POST /documents`
Solicita emissao de documento. **Requer autenticacao (student).**

**Request:**
```json
{
  "type": "transcript",
  "notes": "Preciso para estagio"
}
```
...
### `GET /documents/{id}`
Detalhe do documento (inclui URL para download quando pronto).
...
Slots disponiveis. **Requer autenticacao.**

**Query params:** `?date=2025-01-20&staff_id=uuid`
...
### `POST /appointments`
Agenda atendimento. **Requer autenticacao (student).**
...
Cancela agendamento. **Requer autenticacao.**
...
Webhook que recebe mensagens do WhatsApp Business Cloud API. **Sem autenticacao (validado por signature).**
...
| `create_enrollment` | POST | `/enrollments` | Cria matricula |
